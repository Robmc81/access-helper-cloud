...
export const ConnectorCard = ({ connector, onSync, syncing }: ConnectorCardProps) => {
  ...
  const handleConfigure = () => {
    if (connector.type === "entra") {
      setShowEntraConfig(true);
    } else {
      toast.info("Configuration panel coming soon");
    }
  };

  const handleEntraConfigSave = (config: any) => {
    console.log("Entra configuration saved:", config);
    toast.success("Entra configuration saved");
  };
  ...
  return (
    <>
      ...
      {connector.type === "entra" && (
        <EntraConfig
          isOpen={showEntraConfig}
          onClose={() => setShowEntraConfig(false)}
          onSave={handleEntraConfigSave}
        />
      )}
    </>
  );
